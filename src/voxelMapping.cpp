...
int main(int argc, char **argv) {
    ...
    // noise model params
    nh.param<double>("noise_model/ranging_cov", ranging_cov, 0.04);
    nh.param<double>("noise_model/angle_cov", angle_cov, 0.1);
    nh.param<double>("noise_model/gyr_cov_scale", gyr_cov_scale, 0.5);
    nh.param<double>("noise_model/acc_cov_scale", acc_cov_scale, 1.0);
    ...
    // preprocess params
    nh.param<double>("preprocess/blind", p_pre->blind, 1);
    nh.param<bool>("preprocess/calib_laser", calib_laser, false);
    nh.param<int>("preprocess/lidar_type", p_pre->lidar_type, OUSTER64);
    nh.param<int>("preprocess/scan_line", p_pre->N_SCANS, 128);
    ...
    std::string fileName = "voxelMap.txt";
    std::string seq;
    nh.param<string>("common/seq", seq, "seq");
    std::string filePath = "/home/xingyu/catkin_ws/output/experiments/" + seq.append("/voxelMap/voxelMap.txt");
    std::ofstream ofsTrans(filePath);
    std::cout << "opening stream at: " << filePath << std::endl;
    bool firstScan = true;
    if (!ofsTrans.is_open()) {
        std::cerr << "Failed to open the file for writing." << std::endl;
        return 1;
    }
    Eigen::Matrix4d trans0 = Eigen::Matrix4d::Identity();
    ...
    while (status) {
        ...
        if (sync_packages(Measures)) {
            ...
            Eigen::Quaterniond qt_trans;
            qt_trans.w() = geoQuat.w;
            qt_trans.x() = geoQuat.x;
            qt_trans.y() = geoQuat.y;
            qt_trans.z() = geoQuat.z;

//            std::cout << "quaterniond: " << qt_trans.toRotationMatrix() << std::endl;

            trans.setIdentity();
            trans.block<3, 3>(0, 0) = qt_trans.toRotationMatrix();
            trans(0, 3) = state.pos_end(0);
            trans(1, 3) = state.pos_end(1);
            trans(2, 3) = state.pos_end(2);
            if (firstScan) {
                trans0 = trans;
                trans = Eigen::Matrix4d::Identity();
                firstScan = false;
            } else {
                trans = trans0.inverse() * trans;
            }
//            if (scanIdx == 1 ) {
//                trans0 = trans;
//                trans.setIdentity();
//            }
//            else trans = trans0.inverse() * trans;

            ofsTrans  << trans(0,0) << " " << trans(0,1) << " " << trans(0,2) << " " << trans(0, 3) << " "
            ...
            std::cout << "RAM usage is: " << resident_set / 1000.0 << std::endl;
    ...
    ofsTrans.close();
    std::cout << "ofsTrans saved" <<std::endl;
    return 0;
}
